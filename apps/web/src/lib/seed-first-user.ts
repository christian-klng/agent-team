 ...
 * kein Nutzer existiert (Onboarding auf Coolify ohne manuelles Seed-Script).
 * Wird aus instrumentation.ts nur im Node-Runtime geladen.
 */
export function seedFirstUserIfMissing(): void {
  ...
  void seedLoop(email, password).catch((err) =>
    console.error("[web] Erst-Nutzer-Seed fehlgeschlagen:", err),
  );
}

async function seedLoop(email: string, password: string): Promise<void> {
